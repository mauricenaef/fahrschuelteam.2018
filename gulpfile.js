...
var fahrschuelteam = 'fahrschuelteam.2018';
...
gulp.task('build-dev', [
	...
	connect.server({
		...
		browserSync({
			proxy : 'https://fahrschuelteam.development',
			notify: {
				styles: {
					top: 'auto',
					bottom: '0'
...
gulp.task('dist', function () {
	gulp.src('app/public/wp-content/themes/' + fahrschuelteam + '/**')
		.pipe(gulp.dest('dist/themes/' + fahrschuelteam))
});

gulp.task('copy-theme-dev', function () {
	gulp.src("src/theme/**")
		.pipe(gulp.dest('app/public/wp-content/themes/' + fahrschuelteam));
});
...
gulp.task('style-dev', function () {
	return gulp.src('src/scss/style.scss')
		...
		.pipe(sourcemaps.write('../' + fahrschuelteam))
		.pipe(gulp.dest('app/public/wp-content/themes/' + fahrschuelteam))
...
gulp.task('login-style-dev', function () {
	return gulp.src('src/theme/inc/style/login.scss')
		...
		.pipe(sourcemaps.write('../' + fahrschuelteam))
		.pipe(gulp.dest('app/public/wp-content/themes/' + fahrschuelteam + '/inc/style'))
		.pipe(browserSync.stream({ match: '**/*.css' }));
});
...
gulp.task('login-style-prod', function () {
	return gulp.src('src/theme/inc/style/login.scss')
		...
		.pipe(gulp.dest('dist/themes/' + fahrschuelteam + '/inc/style'))
});
...
gulp.task('header-scripts-dev', function () {
	return gulp.src(headerJS)
		...
		.pipe(gulp.dest('app/public/wp-content/themes/' + fahrschuelteam + '/js'));
});
...
gulp.task('header-scripts-prod', function () {
	return gulp.src(headerJS)
		...
		.pipe(concat('header-bundle.js'))
		.pipe(uglify())
		.pipe(gulp.dest('dist/themes/' + fahrschuelteam + '/js'));
});
...
gulp.task('copy-fonts-dev', () => {
	gulp.src('src/theme/fonts/**')
		.pipe(gulp.dest('app/public/wp-content/themes/' + fahrschuelteam + '/fonts'))
});

gulp.task('copy-fonts-prod', () => {
	gulp.src('src/theme/fonts/**')
		.pipe(gulp.dest('dist/themes/' + fahrschuelteam + '/fonts'))
});
/*********************
Images
*********************/
...
gulp.task('process-images', ['svg-icons'], function() {
    ...
    .pipe(gulp.dest('dist/themes/' + fahrschuelteam + '/images'));
});
